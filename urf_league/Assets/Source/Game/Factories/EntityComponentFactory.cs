        ...
        public static EntityComponent RequestComponent(string componentId, IEntity attachedTo)
        {
            switch (componentId)
            {
                case DataConfig.CHAMP_REND_COMP : 
                    return new PositionComponent(componentId, attachedTo);

                case DataConfig.MOUSE_CONTR_COMP : 
                    return new MouseControlComponent(componentId, attachedTo);

                case DataConfig.MOVEMENT_COMP : 
                    return new MovementComponent(componentId, attachedTo);

                case DataConfig.REND_ORIENT_COMP : 
                    return new RenderOrientationComponent(componentId, attachedTo);
